...
class SSEPendingLinkingStudiesArticleQueue extends Component {
    ...
    swalCallbackAssignJunior(isConfirm, articleId) {
        if(isConfirm)
            this.props.assignSSEPendingLinkingStudiesArticlesJuniorAppraiser(articleId);
    }

    swalCallbackAssignSenior(isConfirm, articleId) {
        if(isConfirm)
            this.props.assignSSEPendingQLinkingStudiesArticlesSeniorAppraiser(articleId);
    }
                ...
                return (
                    <tr key={article[1]._id}>
                        ...
                        */}
                        ...
                        <td>
                            {article[1]._linkingStudiesSeniorEmail || <Link to="/sse/assignedlinkingstudiesarticlequeue" ><Swal options={this.state.swalOptionSenior} callback={ (isConfirm) => this.swalCallbackAssignSenior(isConfirm, article[1]._id)} className="mr-1 badge badge-primary">Assign</Swal></Link>}
                        </td>
